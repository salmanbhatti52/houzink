...
class RealtorInformationDisplayPageState
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: GenericTextWidget(
          title ?? UtilityMethods.getLocalizedString("realtor_info"),
          style: TextStyle(
              fontSize: AppThemePreferences.filterPageResetFontSize,
              fontWeight: AppThemePreferences
                  .propertyDetailsPagePropertyTitleFontWeight,
              color: widget.isDark
                      ...
                      _currentSelection == 0
                          ? ReviewsWidget(
                              ...
                              padding: EdgeInsets.fromLTRB(
                                  20, tabList.length == 1 ? 20 : 5, 20, 0),
                            )
                          : Container(),
                    if (showPropertyListings)
                      headingWidget(
                          text: UtilityMethods.getLocalizedString("listings")),
                    listings(_futureProperties!),
                    if (showAgentAgency)
                      headingWidget(
                          text: isAgent
                              ? UtilityMethods.getLocalizedString("agency")
                              : UtilityMethods.getLocalizedString("agents")),
          ...
          Padding(
            ...
            child: GenericTextWidget(
              ...
              style: TextStyle(
                ...
                color: AppThemePreferences().appTheme.heading01TextStyle!.color,
              ),
            ),
          ),
      ],
            ...
            child: Row(
              children: [
                Expanded(
                  flex: 1,
                  child: SvgPicture.asset(
                    AppThemePreferences.pdLocationImagePath,
                  ),
                ),
                Expanded(
                  flex: 9,
  ...
  Widget taxNumberWidget() {
    return taxNumber != null && taxNumber.isNotEmpty
        ? Container(
  ...
  Widget serviceAreasAndSpecialitiesWidget() {
    return (agentServiceAreas != null && agentServiceAreas.isNotEmpty) ||
            (agentSpecialties != null && agentSpecialties.isNotEmpty)
        ? Container(
            padding: const EdgeInsets.fromLTRB(20, 5, 20, 10),
            child: Column(
              children: [
                agentServiceAreas != null && agentServiceAreas.isNotEmpty
                    ? Container(
  ...
  Widget tabBarViewWidget() {
    return tabList != null && tabList.isNotEmpty
        ? Container(
            ...
            child: Column(
              children: [
                ...
                mobileNumber != null && mobileNumber.isNotEmpty
                    ...
                    : Container(),
                faxNumber != null && faxNumber.isNotEmpty
                    ? SizedBox(
                        height: 40,
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 20),
  ...
  Widget listings(Future<List<dynamic>> articlesList) {
    return Consumer<ItemDesignNotifier>(
      builder: (context, itemDesignNotifier, child) {
        return _currentSelection == tempIndexListing
  ...
  Widget moreElevatedButtonWidget() {
    return realtorListings.length > 2 && _currentSelection == tempIndexListing
        ? SizedBox(
            // width: 140, //160
            child: Container(
              width: double.infinity,
  ...
  Widget realtorInformationDisplayWidget(Future<List<dynamic>>? articlesList) {
    ...
    return _currentSelection == tempIndexRealtor || showAgentAgency
        ? FutureBuilder<List<dynamic>>(
            future: articlesList,
            builder: (context, articleSnapshot) {
              if (articleSnapshot.hasData) {
