...
class _PanelPropertiesListingWidgetState extends State<PanelPropertiesListingWidget> {
  ...
  final Map<String, dynamic> sortByMenuOptions = {
    ...
    "Featured First": Icons.star_outline_outlined,
        ...
        if (articleSnapshot.hasData) {
          ...
          if (_sortFlag == true) {
            ...
            if (_currentPropertyListSortValue == 2) {
              List<dynamic> featuredPropertiesList = propertyList
                  .where((element) =>
              element.propertyInfo!.isFeatured ?? false)
                  .toList();
              if (featuredPropertiesList.isNotEmpty) {
                var set1 = Set.from(propertyList);
                var set2 = Set.from(featuredPropertiesList);
                List<dynamic> newPropertiesList =
                List.from(set1.difference(set2));
                featuredPropertiesList.addAll(newPropertiesList);
                propertyList = featuredPropertiesList;
              }
              // else {
              //   print("No featured property found.............");
              // }
            }
            ...
            if (_currentPropertyListSortValue == 4) {
              propertyList.sort((a, b) {
                int aPrice = UtilityMethods.getCleanPriceForSorting(
                    a.propertyInfo.price.isNotEmpty
                        ? a.propertyInfo.price
...
// TODO: FILTER SEARCH RESULT WIDGET
