...
import 'package:houzi_package/dataProvider/houzez_api_provider.dart';
...
class PropertyRepository{
  ...
  Future<Map<String, dynamic>> fetchPropertyMetaData() async {
    ...
    if(response != null && response.statusCode == 200 && response.data != null && response.data is Map){
      ...
      if(touchBaseDataMap.isNotEmpty){
        // City Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyCityDataType) ?? [];
        ...
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyTypeDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
        ...
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyTypesMetaData(listDataHolder);
          dataHolder = UtilityMethods.getParentAndChildCategorizedMap(metaDataList: listDataHolder);
          if(dataHolder.isNotEmpty){
            HiveStorageManager.storePropertyTypesMapData(dataHolder);
          }
        }
        // Property Country Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyCountryDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyCountriesMetaData(listDataHolder);
        }
        // Property State Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyStateDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyStatesMetaData(listDataHolder);
        }
        // Property Area Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyAreaDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyAreaMetaData(listDataHolder);
        }
        // Property Label Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyLabelDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyLabelsMetaData(listDataHolder);
        }
        // Property Status Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyStatusDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
        ...
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: propertyFeatureDataType) ?? [];
        listDataHolder = parseMetaDataList(dataHolder);
        if(listDataHolder.isNotEmpty){
          HiveStorageManager.storePropertyFeaturesMetaData(listDataHolder);
        }
        // Schedule Time Slots Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: scheduleTimeSlotsKey);
        if(dataHolder != null) {
          HiveStorageManager.storeScheduleTimeSlotsInfoData(dataHolder);
        }
        // Default Currency Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: defaultCurrencyKey);
        if(dataHolder != null) {
          HiveStorageManager.storeDefaultCurrencyInfoData(dataHolder);
        }
        // Enquiry Type Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: enquiryTypeKey);
        if(dataHolder != null) {
          HiveStorageManager.storeInquiryTypeInfoData(dataHolder);
        }
        // User Roles Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: userRolesKey);
        if(dataHolder != null) {
          HiveStorageManager.storeUserRoleListData(dataHolder);
        }
        // All User Roles Meta Data
        dataHolder = UtilityMethods.getListItemValueFromMap(inputMap: touchBaseDataMap, key: allUserRolesKey);
        if(dataHolder != null) {
          HiveStorageManager.storeAdminUserRoleListData(dataHolder);
        }
        // Property Reviews Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: propertyReviewsKey) ?? "";
        ...
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: currencyPositionKey) ?? "";
        if(dataHolder.isNotEmpty) {
          CURRENCY_POSITION = dataHolder;
        }
        // Thousands Separator Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: thousandsSeparatorKey) ?? "";
        if(dataHolder.isNotEmpty) {
          THOUSAND_SEPARATOR = dataHolder;
        }
        // Decimal Point Separator Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: decimalPointSeparatorKey) ?? "";
        if(dataHolder.isNotEmpty) {
          DECIMAL_POINT_SEPARATOR = dataHolder;
        }
        // Add Property GDPR Enabled Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: addPropGDPREnabledKey) ?? "";
        if(dataHolder.isNotEmpty) {
          ADD_PROP_GDPR_ENABLED = dataHolder;
        }
        // Measurement Unit Global Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: measurementUnitGlobalKey) ?? "";
        if(dataHolder.isNotEmpty) {
          MEASUREMENT_UNIT_GLOBAL = dataHolder;
        }
        // Measurement Unit Text Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: measurementUnitTextKey) ?? "";
        if(dataHolder.isNotEmpty) {
        ...
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: leadPrefixKey);
        if(dataHolder != null) {
          HiveStorageManager.storeLeadPrefixInfoData(dataHolder);
        }
        // Lead Source Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: leadSourceKey);
        if(dataHolder != null) {
          HiveStorageManager.storeLeadSourceInfoData(dataHolder);
        }
        // Deal Status Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: dealStatusKey);
        if(dataHolder != null) {
          HiveStorageManager.storeDealStatusInfoData(dataHolder);
        }
        // Deal Next Action Meta Data
        dataHolder = UtilityMethods.getStringItemValueFromMap(inputMap: touchBaseDataMap, key: dealNextActionKey);
        if(dataHolder != null) {
  ...
  Future<Response> fetchContactRealtorResponse(Map<String, dynamic> dataMap, String nonce) async {
    final response = await propertyApiProvider.fetchContactRealtorResponse(dataMap, nonce);
    return response;
  }
  ...
  Future<List<dynamic>> fetchAllProperties(String status, int page, int perPage, int userId) async {
    ...
    if(response != null && response.data != null && response.statusCode == 200){
      _propertiesList.addAll(response.data.map((m) => propertyApiProvider.getCurrentParser().parseArticle(m)).toList());
    }else{
      if(response.statusCode == null) {
      ...
      if(metaDataMap != null){
        if(termData is List){
          for(var dataTypeItem in termData){
            List<dynamic> tempList = validateAndStoreTermData(metaDataMap, dataTypeItem);
            if(tempList != null && tempList.isNotEmpty){
              metaDataList.addAll(tempList);
            }
          }
        }else if(termData is String){
          List<dynamic> tempList = validateAndStoreTermData(metaDataMap, termData);
          if(tempList != null && tempList.isNotEmpty){
            metaDataList.addAll(tempList);
          }
        }
  ...
  List<dynamic> validateAndStoreTermData(Map<String, dynamic> metaDataMap, String dataTypeItem){
    List<dynamic> metaDataList = [];
    if(metaDataMap.containsKey(dataTypeItem)){
      var tempMetaData = metaDataMap[dataTypeItem];
      if(tempMetaData != null && tempMetaData is Map && tempMetaData.isNotEmpty){
      ...
      }else if(tempMetaData != null && tempMetaData is List && tempMetaData.isNotEmpty){
        if(dataTypeItem == 'agent_city' || dataTypeItem == 'agent_category'){
          metaDataList = tempMetaData.map((m) => propertyApiProvider.getCurrentParser().parseAgentMetaData(m)).toList();
        }else{
          List<dynamic> tempMetaDataList = [];
          tempMetaDataList = tempMetaData.map((m) => propertyApiProvider.getCurrentParser().parseMetaDataMap(m)).toList();
          if(tempMetaDataList != null && tempMetaDataList.isNotEmpty) {
            ...
            if(dataTypeItem == propertyAreaDataType) {
              ...
              tempDataMap =
                  UtilityMethods.getParentAndChildCategorizedMap(metaDataList: tempMetaDataList);
  ...
  Future<Response> fetchAddOrRemoveFromFavResponse(Map<String, dynamic> dataMap) async {
    final response = await propertyApiProvider.fetchAddOrRemoveFromFavResponse(dataMap);
    return response;
  }

  Future<List<dynamic>> fetchFavProperties(int page, int perPage, String userIdStr) async {
    List<dynamic> _propertiesList = [];
    final response = await propertyApiProvider.fetchFavProperties(page, perPage, userIdStr);
  ...
  Future<List> fetchUserInfo() async {
    ...
    if(response != null && response.data != null && response.statusCode == 200){
      ...
      if(response.data is String){
        ...
        data = propertyApiProvider.getCurrentParser().parseUserInfo(response.data);
      }
      userInfo.add(data);
    }else{
      if(response.statusCode == null) {
  ...
  Future<List> fetchRealtorInfoList(int page, String type) async {
    ...
    if(response != null && response.data != null && response.statusCode == 200){
      if (type == REST_API_AGENT_ROUTE) {
        realtorInfo.add(response.data.map((m) => propertyApiProvider.getCurrentParser().parseAgentInfo(m)).toList());
      } else {
        realtorInfo.add(response.data.map((m) => propertyApiProvider.getCurrentParser().parseAgencyInfo(m)).toList());
      }
    }else{
      if(response.statusCode == null) {
  ...
  Future<Response> fetchUpdateUserPasswordResponse(Map<String, dynamic> dataMap, String nonce) async {
    final response = await propertyApiProvider.fetchUpdateUserPasswordResponse(dataMap, nonce);
    return response;
  }

  Future<List> fetchSingleArticleViaPermaLink(String permaLink) async {
    List<dynamic> singleArticle = [];
    final response = await propertyApiProvider.fetchSingleArticleViaPermaLinkResponse(permaLink);
  ...
  Future<Response> fetchAddRequestPropertyResponse(Map<String, dynamic> dataMap) async {
    final response = await propertyApiProvider.fetchAddRequestPropertyResponse(dataMap);
    return response;
  }
  ...
  Future<Response> fetchAddAgentResponse(Map<String, dynamic> dataMap, String nonce) async {
    final response = await propertyApiProvider.fetchAddAgentResponse(dataMap, nonce);
    return response;
  }

  Future<Response> fetchEditAgentResponse(Map<String, dynamic> dataMap, String nonce) async {
    final response = await propertyApiProvider.fetchEditAgentResponse(dataMap, nonce);
    return response;
  }

  Future<Response> fetchDeleteAgentResponse(Map<String, dynamic> dataMap, String nonce) async {
    final response = await propertyApiProvider.fetchDeleteAgentResponse(dataMap, nonce);
    return response;
  }

  Future<List<dynamic>> fetchUsers(int page, int perPage, String search) async {
    List<dynamic> usersList = [];
    final response = await propertyApiProvider.fetchUsersResponse(page, perPage, search);

    if(response != null && response.data != null && response.statusCode == 200){
      usersList.addAll(response.data.map((m) => propertyApiProvider.getCurrentParser().parseArticle(m)).toList());
    }else{
      if(response.statusCode == null) {
        usersList = [response];
      }
  ...
  Future<Map> fetchUserPaymentStatus() async {
    ...
    Response response = await propertyApiProvider.fetchUserPaymentStatusResponse();
    if(response != null && response.data != null && response.statusCode == 200){
