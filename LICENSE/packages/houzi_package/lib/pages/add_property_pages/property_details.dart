...
class PropertyDetailsState extends State<PropertyDetails> {
  ...
  void initState() {
    ...
    if (tempMap != null) {
      ...
      if(tempMap.containsKey(ADD_PROPERTY_ADDITIONAL_FEATURES)){
        List<Map<String, dynamic>>? list = tempMap[ADD_PROPERTY_ADDITIONAL_FEATURES];
              ...
              Card(
                child: Column(
                  children: [
                    ...
                    _additionalDetailsList.isNotEmpty
                        ? Column(
                            children: _additionalDetailsList.map((dynamicItem) {
                              int itemIndex =
                                  _additionalDetailsList.indexOf(dynamicItem);
                              return GenericAdditionalDetailsWidget(
                                ...
                                genericAdditionalDetailsWidgetListener:
                                  ...
                                  if (removeItem) {
                                    ...
                                    if (mounted) {
                                      setState(() {
                                        _additionalDetailsList[index].dataMap =
                                            itemDataMap;
                                      });
                                    }
                                  }
  ...
  Widget addNumberOfBathrooms(){
    return genericAddRoomsWidget(
      padding: const EdgeInsets.fromLTRB(10, 20, 20, 0),
      labelText: UtilityMethods.getLocalizedString("bathrooms"),//AppLocalizations.of(context).bathrooms,
          ...
          onPressed: (){
            if(mounted) setState(() {
              _additionalDetailsList.add(DynamicItem(
                key: _additionalDetailsList.isNotEmpty
                    ? "Key${_additionalDetailsList.length}"
                    : "Key0",
                dataMap: {
